...
function init(el, { value, modifiers: { prevent, stop } }) {
  ...
  const removeInterval = () =>
    el._interval && (clearInterval(el._interval) || delete el._interval)
  ...
  el.eEnd = e => {
    ...
    if (duration > 200)
      return isPrevent(press, endEvent) && isPrevent(end, endEvent)
    ...
    el._timeout = setTimeout(() => {
      ...
      if (tapped < 3) {
        ...
        if (e.type !== MOUSE_UP) {
          ...
          const { target } = e
          if (
            target.dispatchEvent(clickEv) === false ||
            clickEv.returnValue === false
          )
            return
        }
        ...
        if (
          e.target.dispatchEvent(tapEv) === false ||
          tapEv.returnValue === false
        )
          return
  ...
  el.eStart = e => {
    ...
    e = actualEvent(
      ...
      isMouseDown
        ? prevent
        : /Android 4\./.test(navigator.userAgent) || prevent,
      stop,
    ).event
    ...
    el._interval = setInterval(
      () =>
        el._startTime ? isPrevent(pressing, wrappedEvent) : removeInterval(),
      200,
    )
  }
