  ...
  bind(el, {value, modifiers:{prevent, stop}}) {
    ...
    utils.on(el, EVENTS.start, el.eStart = e => {
      ...
      if (el._moved) {
        ...
        if (false === handler.moveEnd.call(this, endEvent)) return
        ...
        if (!value.x && !value.y) {
          ...
          if (absChangedX < 10) {
            if (changedY > 50) {
              if (false === handler.swipeDown.call(this, endEvent)) return
            } else if (changedY < -50) {
              if (false === handler.swipeUp.call(this, endEvent)) return
            }
          } else if (absChangedY < 10) {
            if (changedX > 50) {
              if (false === handler.swipeRight.call(this, endEvent)) return
            } else if (changedX < -50) {
              if (false === handler.swipeLeft.call(this, endEvent)) return
            }
          }
        }
      } else {
